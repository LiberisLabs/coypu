...
namespace Coypu.Tests.WhenMakingDirectWebRequests
{
    [TestFixture]
    public class WhenSavingAResourceFromTheWeb
    {
        private StubUrlBuilder _stubUrlBuilder;
        ...
        public void SetUp()
        {
            _stubUrlBuilder = new StubUrlBuilder();
            ...
            _browserSession = TestSessionBuilder.Build(_sessionConfiguration, _driver, new SpyTimingStrategy(), new FakeWaiter(), _stubRestrictedResourceDownloader, _stubUrlBuilder);
        }

        [Test]
        public void It_downloads_the_resource_from_the_given_url_and_saves_to_the_given_location()
        {
            StubResourceUrl("/resources/someresource", "http://built.by/url_builder", _stubUrlBuilder);

            _browserSession.SaveWebResource("/resources/someresource", @"T:\saveme\here.please");
