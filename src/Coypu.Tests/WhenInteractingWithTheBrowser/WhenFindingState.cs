...
using Coypu.Tests.WhenMakingBrowserInteractionsRobust;
using Coypu.Timing;
...
namespace Coypu.Tests.WhenInteractingWithTheBrowser
{
    ...
    public class WhenFindingState
    {
        ...
        {
            var state1 = new State(new AlwaysSucceedsQuery<bool>(true, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state2 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state3 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
        ...
        {
            var state1 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state2 = new State(new AlwaysSucceedsQuery<bool>(true, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state3 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
        ...
        public void It_returns_the_state_that_was_found_first_Example_3()
        {
            var state1 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state2 = new State(new AlwaysSucceedsQuery<bool>(false, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
            var state3 = new State(new AlwaysSucceedsQuery<bool>(true, true, TimeSpan.Zero, _sessionConfiguration.RetryInterval));
